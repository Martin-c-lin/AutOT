...
def find_QDs(image, inner_filter_width=20, outer_filter_width=100,threshold=0.6, particle_size_threshold=30, particle_upper_size_threshold=5000):
    
    image = fourier_filter(image, inner_filter_width=inner_filter_width, outer_filter_width=outer_filter_width)
    image = np.float32(normalize_image(image))
    x,y = find_particle_centers(image, threshold=threshold, particle_size_threshold=particle_size_threshold, particle_upper_size_threshold=particle_upper_size_threshold)
